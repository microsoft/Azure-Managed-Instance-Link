...
function New-MiLink {
    ...
    Begin {
        ...
        $currVersion = [System.Version] (Invoke-DbaQuery -query  "SELECT SERVERPROPERTY('ProductVersion') AS BuildNumber" -SqlInstance $SqlInstance)[0]
        ...
        $masterKeyResult = Invoke-DbaQuery -query "SELECT * FROM sys.symmetric_keys WHERE name LIKE '%DatabaseMasterKey%'" -SqlInstance $SqlInstance
        if (!$masterKeyResult) {
        ...
        $alwaysOn = Invoke-DbaQuery -Query "select SERVERPROPERTY('IsHadrEnabled') as IsHadrEnabled" -SqlInstance $SqlInstance
...
"@
        $certCmds = @(
            { Invoke-DbaQuery -query $using:queryCertCreate -SqlInstance $SqlInstance },
            { Invoke-DbaQuery -query $using:queryCertAddIssuer -SqlInstance $SqlInstance },
            { Invoke-DbaQuery -query $using:queryDigiCertCreate -SqlInstance $SqlInstance },
            { Invoke-DbaQuery -query $using:queryDigiCertAddIssuer -SqlInstance $SqlInstance }
        )
        $temp_guid = New-Guid
    ...
    Process {
        ...
        $recoveryModel = Invoke-DbaQuery -Query $queryGetRecoveryModel -SqlInstance $SqlInstance
        if ($recoveryModel -ne 1) {
            Write-Verbose "Set recovery model to FULL for database $DatabaseName [started]"
            Invoke-DbaQuery -Query "ALTER DATABASE [$DatabaseName] SET RECOVERY FULL" -SqlInstance $SqlInstance       
            Write-Verbose "Set recovery model to FULL for database $DatabaseName [completed]"
        }

        $backupHistory = Get-DbaDbBackupHistory -SqlInstance $SqlInstance -Database $DatabaseName
        if (!$backupHistory) {
            ...
            Invoke-DbaQuery -Query "BACKUP DATABASE [$DatabaseName] TO DISK = N'$backupDiskPath'" -SqlInstance $SqlInstance
        ...
        $dbmCert = Invoke-DbaQuery -Query "exec sp_get_endpoint_certificate @endpoint_type = 4" -SqlInstance $SqlInstance
        if (!$dbmCert) {
            ...
            Invoke-DbaQuery -Query $queryCreateCertificate -SqlInstance $SqlInstance
        }
        ...
        $endpointDBM = Invoke-DbaQuery -Query $queryGetEndpointDBM -SqlInstance $SqlInstance
        if (!$endpointDBM) {
...
"@
            Invoke-DbaQuery -Query $queryCreateEndpointDBM -SqlInstance $SqlInstance
        }
        Write-Verbose "Validate DBM endpoint certificate [completed]"
        $dbmCert = Invoke-DbaQuery -Query "exec sp_get_endpoint_certificate @endpoint_type = 4" -SqlInstance $SqlInstance
        ...
        $miCertInSqlServer = Invoke-DbaQuery -Query "Select * from sys.certificates where name = N'$miFQDN'"
        if (!$miCertInSqlServer) {
            Invoke-DbaQuery -query $queryInstanceDBMCertificateImport -SqlInstance $SqlInstance
        }
        ...
        #Invoke-DbaQuery -query "DROP CERTIFICATE [$miFQDN]" -SqlInstance $SqlInstance  
        #Invoke-DbaQuery -query $queryInstanceDBMCertificateImport -SqlInstance $SqlInstance
...
"@
        Invoke-DbaQuery -query $queryAG -SqlInstance $SqlInstance

        # do we need this ?
        $queryPrimaryAGAlter = "ALTER AVAILABILITY GROUP $PrimaryAvailabilityGroup GRANT CREATE ANY DATABASE"
        Invoke-DbaQuery -query $queryPrimaryAGAlter -SqlInstance $SqlInstance
...
"@
        Invoke-DbaQuery -query $queryDAG -SqlInstance $SqlInstance
...
"@
        ...
        while (($getJobLink.State -eq "Running") -and ($tries -le 7)) {
            ...
            $tries = $tries + 1
            Invoke-DbaQuery -query $queryMonitor1 -SqlInstance $SqlInstance
            Invoke-DbaQuery -query $queryMonitor2 -SqlInstance $SqlInstance
            Get-Job -Id $newLinkJob.Id | Tee-Object -Variable getJobLink
            try {
